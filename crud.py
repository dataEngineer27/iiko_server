...
def add_departments(db: Session, lst):
    ...
    for i in lst:
        ...
        query = models.Departments(id=id, parent_id=parent_id, code=code, name=name, type=type_n,
...
def add_groups(db: Session, lst):
    for i in lst:
        if i['visibilityFilter'] is not None:
            ...
            dep_list = None
        query = models.Groups(id=i['id'], deleted=i['deleted'], parent_id=i['parent'], name=i['name'], num=i['num'],
                              code=i['code'], category_id=i['category'], accountingCategory_id=i['accountingCategory'],
                              departments_visibility=dep_list)
        db.add(query)
        db.commit()
...
# def add_nomenclatures(db: Session, lst, new_dict):
#     for i in lst:
#         try:
#             id = i.find('id')
#             id = id.text if id is not None else None
...
#             category = i.find('productCategory')
#             category = new_dict[category.text] if category is not None else None
#             query = models.Nomenclatures(product_id=id,
...
def add_employees(db: Session, lst, dict_department):
    for i in lst:
        ...
        query = models.Employees(id=id, code=code, name=name, role_id=role_id, roles=roles, role_codes=role_codes,
                                 role_code=role_code, department_id=department_code, deleted=deleted, supplier=supplier,
                                 employee=employee, client=client, representStore=representstore)
        db.add(query)
        db.commit()
    return True
...
def add_department_revenue(db: Session, lst, department):
    for i in lst:
        date = i.find('date')
        ...
        query = models.DepartmentRevenue(date=date, product_id=product_id, department_id=department, sum=sum)
        db.add(query)
        try:
            db.commit()
